...
// Mientras seguimos estabilizando la partida manual en desarrollo,
// desactivamos los timeouts automáticos de fase. Dejamos activos solo
// los jobs que no son de turno, como el fallback del minijuego.
...
export class GameService {
  ...
  public async handleAction(
    ...
    if (action.type === 'RECONNECT_PLAYER') {
      ...
      if (currentState.isMinigameActive && currentState.activeConflict) {
        reconnectEmissions.push({
          room: reconnectingPlayerId,
          event: 'server:game:minigame_start',
          data: {
            player1: currentState.activeConflict.player1,
            player2: currentState.activeConflict.player2,
            type: currentState.activeConflict.type ?? 0,
            duration: currentState.activeConflict.duration ?? 15000,
            isDuel: currentState.activeConflict.isDuel,
          },
        });
      }
    }
      ...
      return [
        {
          ...
          data: {
            player1: action.playerId,
            player2: targetId,
            type: currentState.activeConflict.type,
            duration: currentState.activeConflict.duration,
  ...
  public async triggerStarEvent(lobbyCode: string): Promise<SocketEmission[]> {
    ...
    await gameTimeoutsQueue.add(
      ...
      {
        lobbyCode,
        executeAfter: Date.now() + movement.duration,
      },
      {
