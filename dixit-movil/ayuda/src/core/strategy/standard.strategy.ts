...
export class StandardStrategy implements GameModeStrategy {
  ...
  public transition(state: GameState, action: GameAction): GameState {
    ...
    switch (action.type) {
      ...
      // No avanzamos fases por desconexiones/reconexiones transitorias.
      // En pruebas con 2 jugadores esto desincroniza la ronda: basta con que
      // uno se caiga un instante para que el servidor salte a VOTING o SCORING
      // sin esperar la accion real del otro jugador.
      // Los timeouts AFK ya estan cubiertos por BullMQ.
